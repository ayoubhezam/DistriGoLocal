...
> **Status (same day):** 1.1, 1.2, 1.4, 3.1, 3.2, 3.5 and 5.1 are fixed (commits c7fade2–c54b65c). 1.5 is fixed for quantities, prices and payments; a paid amount above the total is **by design** an advance, now shown as such in the Solde cell (d3bf135). Later the same day: **1.6** — one tournée open at a time (the camion is one); **1.7** — a client or supplier goes to the bin only with a solde of exactly 0; **4.1** — the copy taken before an import holds the database alone (`photos_included` false), and a restore of such a copy keeps the phone's photos. Third batch: **2.2** — duplicate names and barcodes are checked against the database (form and repository); **2.3** — a product deleted for good takes its photo files with it, unless another row still shows the same picture; **3.3** — a waiting restore installs on its own thread at startup (`RestoreStartup`), the first database open waits for it, and the main screen shows « Restauration en cours… ». The rest stands as written.
